---
title: "СПО. Ресурсы"
summary: Книги, курсы, сайты
# description: 123
draft: false
math: false
weight: 2
robotsNoIndex: true
showToc: false
---
### Книги

[Чарльз Петцольд. Код](/books/ssw/Petzold_Code.pdf)

[Эндрю Таненбаум, Херберт Бос. Современные операционные системы](/books/ssw/Tanenbaum_ModernOS.pdf)

[Георгий Курячий, Кирилл Маслинский. Операционная система Linux](/books/ssw/OS_Linux_Maslinskyy_Kuryachiy.pdf)

[Роберт Лав. Linux. Системное программирование](/books/ssw/Love_Linux_System_Programming.pdf)

[Роберт Лав. Ядро Linux. Описание процесса разработки](/books/ssw/Love_Linux_Kernel_Development.pdf)

[Денис Колисниченко. Руководство по командам и Shell-программированию в Linux](/books/ssw/Kolisnichenko_Shell_manual.pdf)

[Мендель Купер. Искусство программирования на языке сценариев командной оболочки](/books/ssw/Advanced_Bash_Scripting_Guide.pdf)

[Daniel Bovet, Marco Cesati. Understanding Linux Kernel](/books/ssw/Bovet_Cesati_Understanding_Linux_Kernel.pdf)

[William Shotts. The Linux Command Line](/books/ssw/Shotts_The_Linux_Command_Line.pdf)


### Курсы

[Основы командной строки](https://ru.hexlet.io/courses/cli-basics)

[Основы Linux](https://stepik.org/course/762/promo)

[Linux для разработчиков](https://slurm.io/linux-for-developers)

[Linux Operating System - Crash Course for Beginners](https://youtu.be/ROjZy1WbCIA)

[Системное программирование. Лаборатория Tarantool](https://youtube.com/playlist?list=PLrCZzMib1e9pOdLmE2qtMgL3QMEIrxyu7)

### Сайты

[Linux Survival](https://linuxsurvival.com/)

[Webminal](https://www.webminal.org/)

[Exercism Bash Track](https://exercism.org/tracks/bash/)
